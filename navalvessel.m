function [xdot,U, M] = navalvessel(x, ui, Vc, betaVc)
% Nonlinear maneuvering model in surge, sway, roll and yaw for a multipurpose
% naval vessel. The surge equation is decoupled except a centripetal term.
%
%   [xdot, U, M] = navalvessel(x, ui, Vc, betaVc)
% 
% Inputs:
%   x = [u v p r phi psi]'
%   ui = [ n_c(1) n_c(2) n_c(3) n_c(4) alpha_c(1) alpha_c(2) ]
%   Vc    : Ocean current speed
%   betaVc: Horizontal current direction
% 
%   tau = [Xe Ye Ke Ne]' 
%   
...
%   Ke is the roll external force  
%   Ne is the yaw external force   
%
% Reference: 
...
%
% Code adapted/modified by Andreas Mork Nordsven, 03.12.2025
% Revisions:
%   - Modified to include ocean current
%   - Modified to handle control inputs ui from control allocation algorithm
%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Include irrotational current and relative velocity here (Modified by Andreas M. Nordsven)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% (Modified by Andreas M. Nordsven with code from MSS toolbox)
vessel.K_max = [300e3, 300e3, 655e3, 655e3]';  % Max thrust for each propeller (N)
vessel.n_max = [140, 140, 150, 150]';               % Max propeller speeds i(RPM)
vessel.K_thr = diag(vessel.K_max./vessel.n_max.^2); % Thruster coefficient matrix

vessel.l_x = [37, 35, -51.5/2, -51.5/2]; % Thruster x-coordinates
...
tau_3dof = T_thr * vessel.K_thr * u_thr;    % [surge sway yaw]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Replaced u with u_rel and v with u_rel in Xh, Yh, Kh, Nh below
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Hydrodynamic forces without added mass terms (considered in the M matrix)
Xh  = h.Xuau*u_rel*au+h.Xvr*v_rel*r;

Yh = h.Yauv*au*v_rel + h.Yur*u_rel*r + h.Yvav*v_rel*av + h.Yvar*v_rel*ar + h.Yrav*r*av ...
   + h.Ybauv*b*abs(u_rel*v_rel) + h.Ybaur*b*abs(u_rel*r) + h.Ybuu*b*u_rel^2;

Kh = h.Kauv*au*v_rel +h.Kur*u_rel*r + h.Kvav*v_rel*av + h.Kvar*v_rel*ar + h.Krav*r*av ...
   + h.Kbauv*b*abs(u_rel*v_rel) + h.Kbaur*b*abs(u_rel*r) + h.Kbuu*b*u_rel^2 + h.Kaup*au*p...
   + h.Kpap*p*ap +h.Kp*p +h.Kbbb*b^3-(const.rho_water*const.g*h.gm*h.disp)*b;

Nh = h.Nauv*au*v_rel + h.Naur*au*r + h.Nrar*r*ar + h.Nrav*r*av...
   +h.Nbauv*b*abs(u_rel*b) + h.Nbuar*b*u_rel*ar + h.Nbuau*b*u_rel*au;
