% Dependicies: 
%   - MSS Toolbox
%   - CasADi Toolbox (version 3.7.2) for symbolic MPC and IPOPT solver
%   - Custom functions: 
%       * params_casADi.m
%       * PIDmodified.m
%       * navalvessel_mpc.m
...
sim_t = 50;     % Final simulation time 
...
time_interval1 = 15; %                  (*SET TIMES FOR SWITCH LOGIC HERE*)
time_interval2 = 50;
...
% Only difference is that this code is utilizing casADi and the solver is 
% IPOPT. Thor I. Fossen use SQP/fmincon.
...
% Start MPC
%xx1 = [];    % store the predicted state
u_cl=[];     % and store control actions we will take
...
for k = 1:total_k-1
    ...
    [ts_temp, xs_temp] = ode45(@(t, x0) navalvessel_mpc(x0, ui, Vc, betaVc), [t t+dt], x0);
    x0 = xs_temp(end, :)';
...
hold on;
h2 = plot(obs_y1, obs_x1, 'ro', 'MarkerSize', 10, 'MarkerFaceColor', 'r', 'DisplayName', 'Obstacle 1 Origin');
hold on
...
tol = 1e-3;             % tolerance to handle float precision (≈ 1 mm)
...
if any(viol2)
    fprintf('Ship violated safety distance for Obstacle 2 at:\n');
    fprintf('  t = %.3f s\n', ts(viol2));
end
