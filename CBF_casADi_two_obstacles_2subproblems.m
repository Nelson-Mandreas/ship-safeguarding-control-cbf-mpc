...
%Tune-able parameters-----------------------------------------------------%
cbf_rate = 0.2;     % CBF rate in CBF constraint for obs 1
cbf_rate2 = 0.15;    % CBF rate for obs 2
%-------------------------------------------------------------------------% 
...
sim_t = 70;     % Final simulation time
...
for k = 1:total_k-1
    ...
    [ts_temp, xs_temp] = ode45(@(t, x) navalvessel(x, ui), [t t+dt], x);
    x = xs_temp(end, :)';

    %% TODO: add wind and ocean current in navalvessel.m
